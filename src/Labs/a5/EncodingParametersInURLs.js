...
function EncodingParametersInURLs() {
  ...
  const fetchSum = async (a, b) => {
    const response = await axios.get(`${process.env.REACT_APP_BASE}/a5/add/${a}/${b}`);
  ...
  return (
    <div>
      ...
      <a
        href={`${process.env.REACT_APP_BASE}/a5/calculator?operation=add&a=${a}&b=${b}`}
        className="btn btn-primary"
      >
        Add {a} + {b}
      </a>
      <a
        href={`${process.env.REACT_APP_BASE}/a5/calculator?operation=subtract&a=${a}&b=${b}`}
        className="btn btn-danger"
      >
        Substract {a} - {b}
      </a>
